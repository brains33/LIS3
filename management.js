    ...
    try {
        ...
        const rows = data.map(p => `
            <tr style="border-bottom:1px solid var(--border);">
                ...
                <td style="padding:8px 4px; font-size:.82rem; text-transform:capitalize;">${p.plan}</td>
                <td style="padding:8px 4px; font-size:.82rem; color:var(--green); font-weight:600;">₦${Number(p.amount_ngn).toLocaleString()}</td>
                <td style="padding:8px 4px; font-size:.75rem; color:var(--muted);">${p.reference}</td>
            </tr>`).join('');
...
async function initiateSubscriptionPayment() {
    ...
    // Fetch public key securely from edge function
    try {
